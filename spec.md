...
Compatible with BIP-360 (P2MR / P2QRH) and FIPS-205. No address prefix hardcoded.

### Companion to BIP-360, not a change

This does NOT propose to change BIP-360. BIP-360 defines WHAT a P2MR / P2QRH address is.
This defines HOW to safely store its seed offline. Seed format is out of scope of BIP-360. Address prefix is determined by wallet, not by backup.
...
- **Single-Address:** One file = One key = One P2MR / P2QRH address
...
- `ALG: SLH-DSA-SHA2-128s` = NIST Level 1, sig ~7856 bytes, current default for P2MR / P2QRH
- If BIP-360 later adds 1-byte version to seed (33 bytes), v2 will be 77 bytes -> ~105-106 chars

### Flow

1. `seed = from_hex(...) -> 32 bytes` — decrypted from 6PQ with password
2. `seed -> SLH-DSA.KeyGen() -> (sk, pk) -> address = P2MR / P2QRH(pk)` — wallet derives address
3. `signature = SLH-DSA.Sign(sk, sighash(transaction))` — sign with private key
4. `broadcast(tx + witness)` — pk is revealed only at spend time, chain stores only hash before
...
- Algorithm is identified by descriptor/wallet setting, e.g. `p2mr_slhdsa_sha2_128s(seed)` / `p2qrh_slhdsa_sha2_128s(seed)`
- `ALG` header in backup solves this for heirs/wallets without changing encrypted bytes
- No `bc1z` / `bc1r` hardcoded — prefix-agnostic for future softforks
...
- v1 (current audit build): PBKDF2-HMAC-SHA256 200k + AES-GCM-256, 76 bytes -> strict 104 chars. Minimal WebCrypto, no deps.
